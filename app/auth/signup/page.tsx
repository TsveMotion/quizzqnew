...
export default function SignUp() {
  ...
  const handleCredentialsSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // For demo, we'll just sign in with the demo accounts
      let demoEmail = ''
      if (userType === 'teacher') {
        demoEmail = 'teacher@quizzq.com'
      } else {
        demoEmail = 'student@quizzq.com'
      }
      ...
      if (result?.error) {
        ...
        toast.success(`Welcome to QuizzQ as a ${userType}!`)
        router.push('/dashboard')
      }
    } catch (error) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      await signIn('google', { 
        callbackUrl: `/dashboard?role=${userType}` 
      })
    } catch (error) {
      console.error('Sign up error:', error)
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-accent-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      ...
      >
        <div className="text-center">
          ...
          >
            Join QuizzQ to create and play interactive quizzes
          </motion.p>
        </div>
